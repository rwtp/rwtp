...
interface OrderMetadata {
  ...
  // the keypair associated with the publicKey.
  scope?: string;
}
```
...
Offers to buy orders or sell orders have metadata. Before publishing a user's shipping address and other offer information, you should encrypt it with the `publicKey` in the `OrderMetadata`.
