...
```solidity
mapping(address => mapping(uint32 => Offer))
```
...
After some period of time (the `timeout`), the deal can be settled, sending the payment to the seller. Use `order.confirm(address, uint32)` to do this.
...
If a buyer thinks they've been scammed, they can request a refund. To do so, use `order.refund(address, uint32)`.
