    ...
    <div className="flex flex-col mt-8">
      <div hidden={expanded}>
        <div
          ...
          className="font-bold text-sm mb-1 flex flex-row"
        >
          <ChevronRightIcon className="h-4 w-4 my-auto" /> Buyer Fields:{' '}
          {schemaType == 'default' ? 'Default' : 'Custom'}
        </div>
      </div>
      <div hidden={!expanded}>
        <div
          ...
          className="font-bold text-sm mb-1 flex flex-row"
        >
          <ChevronDownIcon className="h-4 w-4 my-auto" /> Buyer Fields:{' '}
          {schemaType == 'default' ? 'Default' : 'Custom'}
        </div>
        <div>
          Use the{' '}
          ...
          of buyer fields or define your own in{' '}
          <a className="underline" href="https://json-schema.org/">
            JSON Schema
          </a>
          :
        </div>
          ...
          <div>
            <div className="flex flex-col">
              ...
              <input
                ...
                onChange={(e) => setCustomSchemaIPFS(e.target.value)}
              />
            </div>
          </div>
        )}
          ...
          <div>
            <div className="flex flex-col">
              ...
              {customJSONSchema && is_valid_custom_schema(customJSONSchema) ? (
                <div className="text-sm text-green-500">JSON is valid!</div>
              ) : (
                <div className="text-sm text-red-500">Invalid JSON</div>
              )}
            </div>
          </div>
        )}
